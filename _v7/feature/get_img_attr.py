...
def get_image_feature(img_path):
    ...
    img_feature = img_net.features(img.as_in_context(ctx)).asnumpy().reshape(-1)
    ...
    feature = np.append(img_feature, detect_feature)
...
_thread.start_new_thread(output_image_feature, ('train', 3000, 5,))
#output_image_feature(mode='train', val_cut_idx=3000, frame_per_video=5)
_thread.start_new_thread(output_image_feature, ('val', 3000, 5,))
_thread.start_new_thread(output_image_feature, ('test', 0, 5,))
