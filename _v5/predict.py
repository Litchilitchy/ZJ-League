        ...
        with open('data/submit.txt', 'w') as out:
            ...
            for line in f:
                ...
                for i in range(5):
                    ans += q[i]
                    ans += ','
                    rec.write(line[1+i*4]+'>>>>>>>\n')
                    tp = np.argmax(vid_to_ans[vid][i*3].asnumpy())
                    b = np.max(vid_to_ans[vid][i * 3])
                    rec.write('answer is ' + str(idx_to_ans[str(tp)]) +
                          ' with softmax '+str(b)+'\n')
                    tp = np.argmax(vid_to_ans[vid][i * 3+1].asnumpy())
                    b = np.max(vid_to_ans[vid][i * 3+1])
                    rec.write('answer is ' + str(idx_to_ans[str(tp)]) +
                              ' with softmax ' + str(b) + '\n')
                    tp = np.argmax(vid_to_ans[vid][i * 3+2].asnumpy())
                    b = np.max(vid_to_ans[vid][i * 3+2])
                    rec.write('answer is ' + str(idx_to_ans[str(tp)]) +
                              ' with softmax ' + str(b) + '\n')
